...
<div id="contentPage">
    ...
    <ul>
        ...
        <li><a href="/staff/exAdmins">بازنشسته</a></li>
            ...
            مدیران بازنشسته: <?php echo totalexAdmins($con); ?>        </div>
        <br/><br/>
                    <?php
					$arr = array();
					for($x = 6; $x > 0; $x--)
                    ...
                    <table border="0" cellspacing="0" cellpadding="0" class="fullTable">
						...
						<?php
						$query2 = $con->prepare("SELECT users.*, promotes.ID as 'pID', promotes.Target as 'pTarget', promotes.Action as 'pAction', promotes.ActionDesc as 'pActionDesc' FROM `users`,`promotes` WHERE `AdminLevel` = 0 AND `Action` = 1 AND `ActionDesc` = {$x} AND promotes.Target = users.ID ORDER BY promotes.ID ASC;");
						$query2->execute();
						$c = 0;
