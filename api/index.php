...
$rQuery = new QueryServer( "127.0.0.1", 7777 );
...
switch($_GET['method']) {
	case 'serverinfo':
		if(!is_array($aTotalPlayers) || count($aTotalPlayers) == 0) $players = array('0' => 'unreachable');
		else $players = array();
		foreach($aTotalPlayers AS $id => $value){
			$new = array('nickname' => $value['Nickname'], 'level' => $value['Score'], 'latency' => $value['Ping']);
			array_push($players, $new);
		};
		echo json_encode(array('success' => true, 'time' => time(),
		'status' => array(
			'hostname' => $aInformation['Hostname'],
			'mode' => $aInformation['Gamemode'],
			'players' => $aInformation['Players'],
			'maxplayers' => $aInformation['MaxPlayers'],
			'language' => $aInformation['Map'],
			'artwork' => $aServerRules['artwork'],
			'lagcomp' => $aServerRules['lagcomp'],
			'mapname' => $aServerRules['mapname'],
			'version' => $aServerRules['version'],
			'weather' => $aServerRules['weather'],
			'weburl' => $aServerRules['weburl'],
			'worldtime' => $aServerRules['worldtime'],
			'password' => $aInformation['Password'] ? 'Yes' : 'No'
		),
		'players' => $players,
		));
		break;
	case 'userphones':
		$query = $con->prepare("SELECT DISTINCT `Phone` FROM `users` WHERE `Phone` != 0 AND `HoursPlayed` > 0;");
		$query->execute();
		...
		break;	
	default:
		echo json_encode(array('success' => false, 'error' => 'invalid method [method ? get]'));
		break;
}
?>
